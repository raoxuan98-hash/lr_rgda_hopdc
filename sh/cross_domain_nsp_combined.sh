...
echo "Starting LoRA-NSP Cross-Domain Experiments with combined feature combination..."

# 创建总日志目录
MASTER_LOG_DIR="logs/cross_domain_nsp_combined_$(date +%Y%m%d_%H%M%S)"
...
# 运行LoRA-NSP跨域实验 (nsp_weight=0.05) 使用combined特征组合
run_experiment_type "nsp_lora_0.05_combined" "basic_lora" "$GPU_ID" \
    --gamma_kd "0.0" \
    --nsp_weight "0.05" \
    --nsp_eps "0.05" \
...
echo "=========================================="
echo "LoRA-NSP Cross-Domain experiments with combined feature combination completed!"
...
echo "Feature combination type: combined"
