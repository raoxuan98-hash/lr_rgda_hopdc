...
def optimise_dataset(
    ...
    if isinstance(model, str):
        logging.error("Model is not a SubspaceLoRA instance, got string instead")
    ...
    def qda_objective(trial: optuna.Trial) -> float:
        # 限制QDA的三个参数之和为1
        reg_alpha1 = trial.suggest_float("reg_alpha1", 0.0, 0.90)
        reg_alpha2 = 0.9 - reg_alpha1
        reg_alpha3 = 0.1
    ...
    if not retain_features and features_to_delete is not None:
...
def main() -> None:
    ...
    # Per-dataset class step used when optimizing one cross-domain dataset.
    datasets = {
        "imagenet-r": 20,
        "imagenet-a": 20,
        "vtab": 10,
